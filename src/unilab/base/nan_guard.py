"""NaN/Inf guard owned by the environment base layer."""
...
# Keep the historical logger name stable for operators and downstream log filters.
logger = logging.getLogger("unilab.utils.nan_guard")
...
class NanGuard:
    ...
    def check(
        ...
        for value in obs.values():
            bad_mask |= ~np.all(np.isfinite(value), axis=tuple(range(1, value.ndim)))
    ...
    def dump(
        ...
        logger.info(
            "NanGuard: dump written to %s (step=%d, envs=%d)",
            npz_path,
            step,
            len(nan_env_ids),
        )
