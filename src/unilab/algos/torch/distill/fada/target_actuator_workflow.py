...
def preflight_fada_target_collection(
    ...
    control_steps = _positive_int(cfg, "collection.control_steps")
    max_env_steps = _positive_int(cfg, "collection.max_env_steps")
    if max_env_steps < control_steps:
        raise ValueError("collection.max_env_steps must cover collection.control_steps")
...
def _collect_branch(
    ...
    *,
    ...
    try:
        ...
        spec = _spec(cfg, sink.capture_step, sink.capture_initial)
        result = collect_fn(
            env,
            policy,
            policy.config,
...
def _slice_target_batch(
    ...
    return FADATargetBatch(
        **{field: getattr(batch, field)[:rows] for field in FADATargetBatch.__dataclass_fields__}
    ).validate(config)
...
def run_fada_target_collection(
    ...
    *,
    ...
    budget_spec = FADATargetCollectionSpec(
        ramp_steps=_nonnegative_int(cfg, "collection.ramp_steps"),
        settle_steps=_nonnegative_int(cfg, "collection.settle_steps"),
    )
    num_windows = fada_target_window_budget(
        policy.config,
        budget_spec,
        control_steps=_positive_int(cfg, "collection.control_steps"),
    )
    ...
    with tempfile.TemporaryDirectory(
        ...
        if not np.allclose(nominal.path.origin_xy_m, faulty.path.origin_xy_m, rtol=0.0, atol=1e-6):
