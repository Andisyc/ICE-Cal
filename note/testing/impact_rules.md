...
| `G1StandStill` / `task=sac/g1_stand_still/mujoco` | owner task config -> env registry -> zero command sampler -> reward dispatch -> static support sensors -> 98-dim obs/symmetry -> checkpoint/playback dim guard -> live standing stability | S0/S1/S2/S3/S4; T-connect, T-oracle, T-value, T-shape, T-persist, T-live |
| `stand_support_height_*` / `stand_base_height_deficit_l1` / Standing high-support height bundle | reference concept -> stand-still reward lab roles -> G1 standing reward owner -> reward scales in `g1_stand_still` only -> foot contact/slide sensors -> config isolation from Walking/height tasks -> live sentinel per-term logs -> short training/playback height trace | S1/S2/S4; T-value, T-role, T-diff, T-oracle, T-scale, T-live |
...
| distill learner runtime identity | aggregate source paths -> `torch.load/map_location` -> real MoE update -> checkpoint reload -> PersistentDistillationRuntime -> SharedWeightSync -> process/resource cleanup; native anomalies require matched CPU/GPU fresh, GPU continuous/restart/dual owner-path differentials, correctly unpacked core/GDB, and kernel/GPU/RAM health capture. Generic numeric/object canaries are platform auxiliaries only | S1/S2/S3/S4; T-transform, T-oracle, T-diff, T-state, T-update-order, T-persist, T-live |
