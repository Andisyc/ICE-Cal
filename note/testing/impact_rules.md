...
| `stand_support_height_*` / Standing high-support height bundle | reference concept -> stand-still reward lab roles -> G1 standing reward owner -> reward scales in `g1_stand_still` only -> foot contact/slide sensors -> live sentinel per-term logs -> short training/playback height trace | S1/S2/S4; T-value, T-role, T-diff, T-oracle, T-live |
