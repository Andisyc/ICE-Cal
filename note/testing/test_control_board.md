...
| G1LOC-E-001 | `start.sh` | Entrypoint / CLI / config | S0, S2 | T-connect, T-oracle | Existing wrapper is note-confirmed only | missing-test | Need shell contract test or documented manual smoke for arg rewrite. |
...
| G1LOC-STAND-001 | `g1/joystick.py` standing behavior | Reward / metric / evaluator | S1, S2, S4 | T-value, T-role, T-oracle, T-diff, T-live | stand geometry/recovery/action-authority tests | covered-but-live-gap | Stable stand still not runtime-confirmed. |
| G1LOC-ACT-001 | `g1/joystick.py` action and phase execution | Env command / reset / lifecycle adapter | S1, S2, S4 | T-transform, T-oracle, T-live, T-scale | apply_action phase/action authority tests | covered-but-live-gap | Need first-step action/action_scale/default pose live probe. |
...
| --- | --- | --- | --- |
| G1LOC-E-001 | S0/S2: T-connect, T-oracle | Add shell/static test that `./start.sh <run>` expands to `algo.load_run=<run>` and `+algo.checkpoint=model_5000.pt` without launching viewer. | wrapper arg contract. |
...
| G1LOC-ENV-001 | S4: T-live, T-oracle | Short rollout probe prints reset qpos/qvel, base height, tilt, termination reason at step 0/1. | identify direct-fall boundary. |
| G1LOC-RWD-002 | S4: T-live, T-role | Per-term live reward snapshot for first N steps with command role tags. | distinguish bad reward shaping from action/model issue. |
| G1LOC-ACT-001 | S4: T-live, T-scale | First-step policy action probe prints actor action mean/std/min/max, executed action, PD target, action_scale, default_angles head. | detect bad action scale/default pose/action saturation. |
| G1LOC-STAND-001 | S4: T-live, T-diff | Zero-action vs policy-action standing recovery differential rollout. | isolate whether policy destabilizes a physically valid reset. |
