...
| G1LOC-C-004 | `conf/offpolicy/task/sac/g1_stand_still/mujoco.yaml` | Entrypoint / CLI / config | S0, S2, S3 | T-connect, T-oracle, T-shape, T-persist, T-diff | `test_offpolicy_g1_stand_still_is_explicit_expert_contract`; `test_offpolicy_g1_stand_still_pose_anchor_matches_upstream_walking_reward`; `test_g1_stand_still_symmetry_keeps_walking_actor_obs_dim` | covered-but-live-gap | Offline contract confirms no Walking/height/mode route, 98-dim actor obs, upstream G1WalkFlat posture anchor, G1StandStill high-support height bundle ownership, and G1WalkFlat/G1WalkHeight isolation from Standing high-support keys; live standing stability still needs S4 after training. |
...
| G1LOC-RWD-002 | `g1/joystick.py` reward dispatch | Reward / metric / evaluator | S1, S2, S4 for live reward health | T-value, T-role, T-oracle, T-meta, T-diff, T-scale, T-live | reward mode dispatch/gating/log tests in `test_gait_constraint.py`; height-vs-walking reward scale tests in `test_g1_height_tracking_contract.py`; `support-policy-diff` live sentinel | covered-with-live-differential | Live per-term snapshot confirms no forbidden Walking/gait/height keys and shows `stand_support_height_margin_l2` is active. Post-log regression test now guards the actual `base_height≈0.677` low equilibrium; existing checkpoints still need retraining to change actor behavior. |
...
| G1LOC-STAND-001 | `g1/joystick.py` standing behavior | Reward / metric / evaluator | S1, S2, S4 | T-value, T-role, T-oracle, T-diff, T-live | stand geometry/recovery/action-authority tests; static-support tests; `test_g1_stand_still_reward_lab_prefers_clean_stand_over_pose_failures`; `scripts/deploy/check_unilab_g1_stand_still_live_sentinel.py` | covered-with-live-differential | Offline clean stand now outranks bad pose and bad support rows, including the real trained low equilibrium around `0.677m`. `support-policy-diff` live sentinel confirms high-support is physically reachable for 64 steps (`searched_support_action` height min 0.7219, tilt max 11.49 deg, both feet contact 1.0), while zero action fails (`height min 0.5792`, tilt max 38.59 deg). Existing checkpoint remains low because actor weights were trained before the stronger high-support scale. |
...
| G1LOC-STAND-002 | S1/S2: T-value, T-role, T-diff, T-oracle | Reward lab compares high-support stand, low crouch, missing contact, sliding feet, rear lean, and bad base-over-feet geometry. | covered offline for loaded low crouch after `stand_support_height_margin_l2`; live physics remains G1LOC-STAND-003. |
| G1LOC-STAND-003 | S4: T-live, T-diff, T-scale, T-oracle | Live sentinel logs zero action vs searched support action vs trained policy with high-support reward terms. | executed: route/reward keys and short-horizon high-support feasibility runtime-confirmed; long-horizon high-support durability remains the Step 6 gate. |
