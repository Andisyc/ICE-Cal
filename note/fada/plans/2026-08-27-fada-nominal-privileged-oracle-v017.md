...
Remove downstream calibration faults from Oracle ownership. Train the privileged Oracle without
targeted actuator faults; inject execution failures only after the source Oracle and
Planner–Tracker have been trained and frozen.

## Semantic migration

| Object | v017 owner | Retired v016 interpretation | Engineering proof destination |
|---|---|---|---|
| Oracle training distribution | no targeted actuator faults | any targeted execution fault | composed-config absence and runtime preflight |
...
| Targeted actuator faults | downstream failed-rollout collection | Oracle domain randomization | collection identity and isolation tests |
...
1. Disable actuator-strength and every physical/domain randomization in the privileged Oracle owner
   configuration.
2. Make preflight reject any Oracle-side targeted execution fault.
...
All v016 module, formal, command, checkpoint, and policy-quality evidence that conflicts with the
Oracle/downstream fault boundary is historical. The unsuccessful
...
- A targeted actuator fault is visible to or changes the source Oracle lineage.
