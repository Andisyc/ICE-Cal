---
contract_id: FADA-CONTEXT-PHASE1-METHOD-v002
status: superseded
effective_date: 2026-08-10
updated_date: 2026-08-10
supersedes: FADA-CONTEXT-PHASE1-METHOD-v001
prerequisite: FADA-METHOD-v005
scope: privileged residual-teacher feasibility and accepted paired quality gate under controlled 29D actuator-strength variation
---
...
Phase-1 training varies `g` across resets using only three structural strata: nominal strength,
exactly one changed left-knee actuator, or exactly one changed right-knee actuator. Knee multipliers
are sampled continuously in `[0.85, 0.95]`, with nominal probability `0.2`. A run with only one fixed
left-knee value is a playback probe and cannot establish conditional repair.
...
Every report retains nominal, left-knee, and right-knee strata. The accepted formal protocol uses
...
For each of `left_knee` and `right_knee` independently:
...
- Formal training may start only after the v002 training preflight passes without creating a trainer
...
real-robot evaluation, publication novelty, and formal teacher quality remain unconfirmed until the
trained checkpoint passes this gate.
