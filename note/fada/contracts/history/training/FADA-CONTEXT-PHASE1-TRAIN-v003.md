---
contract_id: FADA-CONTEXT-PHASE1-TRAIN-v003
status: superseded
effective_date: 2026-08-10
updated_date: 2026-08-10
supersedes: FADA-CONTEXT-PHASE1-TRAIN-v002
superseded_by: FADA-CONTEXT-PHASE1-TRAIN-v004
method_contract: FADA-CONTEXT-PHASE1-METHOD-v003
scope: two-condition default-off privileged residual SAC teacher route with formal paired acceptance
---
...
and critic observation. The only candidate is left knee index `3`, its multiplier range is fixed to
`[0.9, 0.9]`, nominal probability is `0.5`, unrelated Kp/Kd randomization is disabled, and the
command is fixed to `(0.4, 0.0, 0.0)` m/s. The task adds initial-frame lateral-displacement and yaw-
drift penalties so the training objective directly owns the paired straight-line metrics. Automatic
post-training playback is disabled for the formal run.
...
`FADA-CONTEXT-PHASE1-METHOD-v003`. A checkpoint evaluated with different seeds, environment count,
...
- Do not omit the nominal stratum; anomaly-only training cannot establish conditional repair.
- Do not introduce right-knee or continuous-strength generalization into this feasibility stage.
