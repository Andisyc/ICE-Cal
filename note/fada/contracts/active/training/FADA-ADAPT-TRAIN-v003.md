...
Inputs are a schema-5 source checkpoint and slope `fada-target-batch/v3`, both carrying the exact
...
The deterministic split holds out complete command groups, with episode/time and purged-time
fallbacks retained for legacy single-command/single-trajectory artifacts. Slope adaptation rejects
reused commands across represented episodes before optimizer construction. The LoRA-only update
transaction, frozen Planner/base-IDM ownership, first-action loss, and atomic persistence remain
unchanged. The optimizer owns every and only Q/V LoRA parameter exactly once. A zero adapter must
reproduce the source policy before any optimizer step.
