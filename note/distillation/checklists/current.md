...
| Formal DAgger two-round r1 Gate 0 identity | formal identity + compose connector | S0/S1 / T-workload+lineage+output | BLOCKED | E105: Hydra argv ordering failed before workload observation; training false; r1 permanently closed |
...
| FT-0 formal identity materialization | distill formal-identity owner + deploy connector | S0/S1 / T-identity+compose+oracle | PASS | E105/E108: r2 no-training preflight accepted; no training was executed by Gate 0 |
| FT-1 formal DAgger execution | formal CLI/workflow/runtime/artifact owners | S2/S3/S4 / T-formal+artifact+live | PASS | E108: existing r2 completed two iterations and postflight-v2 accepted artifacts; this is not RT-10 physical acceptance |
| FT-1 acceptance oracle v2 | formal postflight owner | S1/S2/S3 / T-lineage+artifact+runtime-identity | PASS | E108: read-only postflight-v2 accepted existing r2 artifacts; it did not rerun training |
...
| Final fresh r1 Gate 0 materialization | deploy connector + oracle v2 | S1/S3 / T-identity+workload+preflight | PARTIAL | E108: r1 Gate 0 accepted and training later stopped at iteration 4/next aggregate OOM; preserve it, never resume it |
| Formal auto output identity | formal identity + Gate 0 connector | S1/S2 / T-name+freeze+compat | PASS | E109: fixed-clock owner, connector freeze/materialization fixture, manual compatibility, docs/Atlas checks; server Gate 0 remains separately controlled |
| Ordinary `train.sh` fresh/resume launcher | `train.sh` + owner CLI | S1/S2 / T-shell+route-compose | PASS | E112: paired fresh identity, explicit manifest-backed resume, duplicate-route rejection, 5 dry-run tests, owner-CLI compose; no training |
| Formal fresh r2 local resource spec | formal identity + workflow config | S0/S1/S2 / T-spec+schedule+owner-compose | PASS | E110: `run_name` spec, 64->32 collector containment, unchanged 147456-update workload, fixed-clock identity and owner compose PASS; no server Gate 0/training |
| Formal fresh r2 Gate 0 materialization | deploy connector + oracle v2 | S3 / T-server-identity+workload+preflight | PASS | E111: accepted server preflight, returncode 0, one `20260720-140520` auto identity, `training_executed=false`; supervisor/training remain closed |
| Formal fresh r2 FT-1 frozen supervisor/oracle | frozen supervisor + matching frozen oracle | S4 / T-live+artifact+postflight | IN PROGRESS | `plans/formal_dagger_fresh_8iter_r2_ft1_execution.md`: one authorized invocation, `&&` oracle only after success, no retry/resume |
