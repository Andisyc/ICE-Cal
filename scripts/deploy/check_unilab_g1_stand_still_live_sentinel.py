...
def _contact_sensor_count(env: Any, prefix: str) -> np.ndarray:
    ...
    return np.sum(np.stack(values, axis=1) > 0.5, axis=1).astype(np.float32)
...
def _snapshot(env: Any, state: Any, *, include_termination: bool = True) -> dict[str, Any]:
    ...
    out = {
        "base_height": _stats(_height(env)),
...
def _snapshot_by_role(env: Any, state: Any, roles: np.ndarray) -> dict[str, Any]:
    ...
    support = _support_metrics(env)
    return {
        "base_height": _role_stats(_height(env), roles),
...
def _stability_score(
    ...
    *,
    ...
    height = _height(env)
    foot = _foot_metrics(env)
    base_x = np.abs(foot["base_over_feet_x"])
    tilt_cost = tilt / max(max_ok_tilt_deg, 1.0)
    height_cost = np.maximum(0.0, min_ok_base_height - height) / max(min_ok_base_height, 1.0e-6)
    ...
    return -(tilt_cost + 4.0 * height_cost + 2.0 * base_x_cost + termination_cost)
    ...
    *,
    ...
    for iteration in range(iterations):
        ...
        current = {
            ...
            "best_action_l2": float(np.linalg.norm(samples[top_idx])),
...
def run_check(
    *,
    ...
    try:
        ...
        if probe_mode == "anchor-diff":
            role_final = details["rollout"]["final_by_role"]
            zero_tilt = role_final["tilt_deg"]["zero_action"]["max"]
            hold_tilt = role_final["tilt_deg"]["hold_reset_pose"]["max"]
            zero_height = role_final["base_height"]["zero_action"]["min"]
            hold_height = role_final["base_height"]["hold_reset_pose"]["min"]
            zero_x = max(
                abs(role_final["base_over_feet_x"]["zero_action"]["min"]),
                abs(role_final["base_over_feet_x"]["zero_action"]["max"]),
            )
            hold_x = max(
                abs(role_final["base_over_feet_x"]["hold_reset_pose"]["min"]),
                abs(role_final["base_over_feet_x"]["hold_reset_pose"]["max"]),
            )
            details["module_probe"]["boundary_decision"] = {
                "zero_action": {
                    "tilt_max": zero_tilt,
                ...
                "hold_reset_pose": {
                    "tilt_max": hold_tilt,
                    "height_min": hold_height,
                    "base_over_feet_x_abs_max": hold_x,
                },
                "interpretation": (
                    "G1LOC-ACT-001 default_angles/static action anchor mismatch"
                    if hold_tilt + 10.0 < zero_tilt or hold_x + 0.1 < zero_x
...
def main() -> int:
    ...
    parser.add_argument(
        "--probe-mode", choices=("zero", "anchor-diff", "anchor-search"), default="anchor-diff"
    )
