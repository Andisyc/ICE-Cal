...
run_dir="$PWD/logs/FADAPrivilegedOracle_velocity_stage1_${run_stamp}"
...
export ICE_CAL_ORACLE_LINEAGE_ID="velocity-stage1-${run_stamp}"
taskset -c "0-${last_cpu}" nice -n 5 ionice -c2 -n5 \
  uv run --frozen --no-sync python scripts/train_offpolicy.py \
  algo=sac \
  task=sac/g1_walk_flat/mujoco_fada_privileged_oracle_velocity_stage1 \
